...
{ void *file=(void*)CreateFileA(name,GENERIC_WRITE|GENERIC_READ,FILE_SHARE_WRITE|FILE_SHARE_READ,0x00,CREATE_ALWAYS,0x00,0x00);
	return ccisfile(file)?file:0;
}

ccfunc void *
ccpullfile(void *file, unsigned long int offset, unsigned long int *lplength)
{ if(!ccisfile(file)) return 0;
